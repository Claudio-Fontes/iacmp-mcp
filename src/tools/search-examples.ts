...
function formatResult(ex: SearchResult): string {
  ...
  const notes: string[] = Array.isArray(ex.content.notes)
    ? ex.content.notes
    : (ex.content.notes ? [ex.content.notes as unknown as string] : []);
  if (notes.length > 0) {
...
export function handleSearchExamples(args: { query: string; provider?: string; limit?: number }): string {
