import { searchExamples, getExampleById, ALL_EXAMPLES, type Example } from '../knowledge/index.js';
...
export function handleSearchExamples(args: { query: string; limit?: number }): string {
...
export function handleGetExample(args: { id: string }): string {
  const ex = getExampleById(args.id);
  if (!ex) return `Exemplo "${args.id}" não encontrado. IDs disponíveis: ${ALL_EXAMPLES.map(e => e.id).join(', ')}`;
  return formatExample(ex);
}

export function handleListExamples(): string {
